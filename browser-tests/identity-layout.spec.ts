...
test("top-right logos reserve header space on stage and previews without an identity footer", async ({
  browser,
}, testInfo) => {
  ...
  try {
    ...
    for (const step of deck.steps) {
      ...
      for (const surface of [stage, preview]) {
        await expect(surface.locator(".presentation-identity")).toHaveCount(0);
        await expect(surface.locator(".identity-top-logo")).toHaveCount(
          step.hideIdentity ? 0 : 1,
        );
        if (!step.hideIdentity) {
          const geometry = await surface.locator(".stage").evaluate((root) => {
            const logo = root
...
test("header logos fit presentation and publication PDFs including demo frames", async () => {
