...
export function audienceStage(state:Partial<Stage>):Partial<Stage> {
 ...
 for (const key of ["live","act","mode","title","html","source","diagram","demoUrl","version","theme","blank","build"] as const) {
   if (state[key] !== undefined) Object.assign(result,{[key]:structuredClone(state[key])});
 }
 if (result.mode === "demo") {
   let publicUrl = false;
...
export class AudienceStageSync {
 origin?:string;token?:string;fetcher:Fetcher;sent:string;pending:string;busy:boolean;error:string;closed:boolean;timer?:ReturnType<typeof setTimeout>|null;

 constructor({origin,token,fetcher=fetch}:{origin?:string;token?:string;fetcher?:Fetcher}={}) { this.origin=origin;this.token=token;this.fetcher=fetcher; this.sent="";this.pending="";this.busy=false;this.error="";this.closed=false; }
 ...
 close(){this.closed=true;clearTimeout(this.timer ?? undefined);}
}
