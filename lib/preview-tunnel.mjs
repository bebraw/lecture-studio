...
export class PreviewTunnel {
 constructor({binary=process.env.LECTURE_CLOUDFLARED_BIN || "cloudflared",spawnProcess=spawn,timeout=30000}={}) {
  Object.assign(this,{binary,spawnProcess,timeout});this.url="";this.target="";this.pending=null;
 }
 async open(value,studioOrigin) {
  ...
  try {
   const ready=new Promise((resolve,reject)=>{
    ...
    const finish=(error)=>{clearTimeout(timer);this.cancel=null;error?reject(error):resolve(publicOrigin);};
