...
export class AudiencePoll {
 constructor({origin=process.env.LECTURE_POLL_ORIGIN,room=process.env.LECTURE_POLL_ROOM || "webdev-2026",token=process.env.LECTURE_POLL_TOKEN,fetcher=fetch}={}) {
   this.fetcher=fetcher;this.token=token;this.config=themePoll();this.snapshot=null;this.frozen=null;this.busy=false;this.error="";
   if(origin){
 ...
 state(){return {configured:!!this.origin && !!this.token,config:this.config,snapshot:this.snapshot,frozen:this.frozen,busy:this.busy,error:this.error,joinUrl:this.origin ? this.origin+"/rooms/"+this.room : ""};}
 configure(config){
 ...
 reset(){if(this.busy)throw new Error("Wait for the poll operation");this.snapshot=null;this.frozen=null;this.error="";}
}
