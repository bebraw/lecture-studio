      ...
      if (!this.state.threadId) {
        const result = await this.rpc("thread/start", {
          ...
          developerInstructions:
            ...
            "LECTURE DEMO MODE: The lecturer authorizes a focused verification policy for these local demo increments, overriding the starter's general full-quality-gate completion requirement. Run typecheck for TypeScript changes, tests covering changed behavior, and one browser smoke check of the demonstrated interaction. Honor the audience-selected acceptance test first. Preserve input validation, authorization, privacy, native form behavior and composition output validation. Reuse installed dependencies, the existing dev server and prepared room infrastructure; reinstall only when dependencies change or are missing. Do not run full CI, whole-project coverage, dependency audits, mutation testing, Lighthouse, broad capability-kit verification or unrelated diagnostics during the lecture unless directly needed to investigate a failure. Defer broad regression and release checks until after the lecture; do not claim release readiness. An explicit deployment or release-readiness request requires the full project quality gate before deployment. Keep documentation to a short record of changed behavior, checks and deferred work; defer unrelated architecture documents and template maintenance. Start or reuse the preview as soon as the increment works, then finish focused checks. Stop when the requested increment and its focused checks are complete; avoid extra polishing or building the next increment. Report the preview URL, checks passed, and deferred full validation concisely.",
        });
