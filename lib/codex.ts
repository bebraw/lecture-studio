...
import type {BridgeState,ApprovalRequest,ApprovalParams} from "../shared/models.ts";
type SpawnProcess=(command:string,args:string[],options:SpawnOptions)=>ChildProcess;
interface RpcResults {
 initialize:unknown;
 "model/list":{data?:{id:string;model?:string;displayName?:string;isDefault?:boolean;hidden?:boolean}[]};
...
interface ProtocolParams extends ApprovalParams {
 threadId?:string;turn?:{id?:string;status?:string;error?:{message?:string}};
 itemId?:string;delta?:string;item?:{id:string;type:string;text?:string};error?:{message?:string};
}
interface ProtocolMessage {id?:string|number;method?:string;params?:ProtocolParams;result?:unknown;error?:{message?:string}}
...
function stopOwnedProcess(proc:ChildProcess|undefined|null) {
 ...
 if (process.platform !== "win32" && proc.pid) {
   try { process.kill(-proc.pid, "SIGTERM"); return; } catch { /* Already stopped or no owned group. */ }
 }
 proc.kill("SIGTERM");
}

export class CodexBridge extends EventEmitter {
 binary:string;spawnProcess:SpawnProcess;pending:Map<string|number,{resolve:(value:unknown)=>void;reject:(error:Error)=>void;timer:ReturnType<typeof setTimeout>}>;approvals:Map<string|number,ApprovalRequest>;
 sequence:number;state:BridgeState;proc?:ChildProcess;connecting=false;busy=false;

 constructor({ binary = process.env.LECTURE_CODEX_BIN || "codex", spawnProcess = spawn }: {binary?:string;spawnProcess?:SpawnProcess} = {}) {
 ...
 async connect(workspace:string) {
   ...
   const proc = this.spawnProcess(this.binary, args, { cwd: path, env, stdio: ["pipe", "pipe", "pipe"], shell: false, detached: process.platform !== "win32" });
 ...
 rpc<M extends keyof RpcResults>(method:M, params:unknown, timeout = 30000):Promise<RpcResults[M]> {
   ...
   return new Promise<RpcResults[M]>((resolve, reject) => {
     ...
     try { this.send({ id, method, params }); } catch (caught) { const error = asError(caught); clearTimeout(timer); this.pending.delete(id); reject(error); }
   });
 }
 receive(message:ProtocolMessage) {
   ...
   if (method === "turn/started") { this.state.turnId = params.turn?.id ?? null; this.state.status = "working"; this.state.activity = "Working"; }
   ...
   if (method === "item/agentMessage/delta") this.appendMessage(params.itemId ?? "message", params.delta || "");
   if (method === "item/started") {
     const labels:Record<string,string> = { commandExecution: "Running a command", fileChange: "Editing files", mcpToolCall: "Using a tool", webSearch: "Looking up a source", agentMessage: "Responding" };
 ...
 async start(prompt:string, model = "") {
   ...
   try {
     if (!this.state.threadId) {
       const result = await this.rpc("thread/start", { cwd: this.state.workspace, sandbox: "workspace-write", approvalPolicy: "on-request", approvalsReviewer: "auto_review",
     ...
     this.state.messages = [];
     const result = await this.rpc("turn/start", { threadId: this.state.threadId, approvalPolicy: "on-request", approvalsReviewer: "auto_review", input: [{ type: "text", text: prompt }], ...(model ? { model } : {}) });
     if (this.state.status === "working") this.state.turnId = result.turn?.status === "completed" ? null : result.turn?.id ?? null;
     this.state.model = model || "Codex default"; this.changed();
   } catch (caught) { const error = asError(caught); this.state.status = "failed"; this.state.activity = "Start uncertain or failed · reconnect before retry"; this.changed(); throw error; }
 ...
 answer(id:string|number, decision:string, answers?:Record<string,string>) {
   const item = this.approvals.get(id); if (!item) throw new Error("This request is no longer pending");
   let result: {answers:Record<string,{answers:string[]}>} | {permissions:Record<string,never>;scope:string} | {decision:string};
 ...
 fail(message:string) {
   ...
   const child = this.proc; this.proc = undefined; stopOwnedProcess(child);
 ...
 close() {
   const proc = this.proc; this.proc = undefined; stopOwnedProcess(proc);
