...
const impacts: Record<string, string> = {
  finding:
    "Make the seminar essentials easy to scan, with clear headings and a concise summary.",
  repeating:
    "Preserve the audience's choice when enhancing the form; do not require repeat entry.",
  navigation: "Use descriptive links and a simple, predictable page structure.",
  trust:
    "Clearly attribute seminar facts and distinguish source content from generated summaries.",
  editorial:
    "Use an editorial visual theme with restrained typography and clear hierarchy.",
  "retro-web":
    "Use a readable retro-web visual theme while preserving accessible contrast and native controls.",
  playful:
    "Use a playful visual theme while preserving readable typography and accessible controls.",
  overview: "Prioritize a concise overview using only trusted seminar data.",
  learning:
    "Prioritize supported learning outcomes; do not invent promises absent from the source data.",
  practical:
...
export class AudiencePoll {
  ...
  error: string;
  pollId: string;
  rounds: Record<string, PollRound>;
  baseRoom: string;
  origin?: string;
  room: string = "";

  constructor({
    origin = process.env.LECTURE_POLL_ORIGIN,
    room = process.env.LECTURE_POLL_ROOM || "webdev-2026",
    token = process.env.LECTURE_POLL_TOKEN,
    fetcher = fetch,
    ...
    if (origin) {
      ...
      if (
        ...
        throw new Error(
          "Poll origin must be HTTPS or local HTTP, without a path or credentials",
        );
  ...
  decisions(): PollState["decisions"] {
    ...
    for (const [id, round] of Object.entries({
      ...this.rounds,
      [this.pollId]: { frozen: this.frozen },
    })) {
      if (round.frozen)
        decisions[id] = {
          ...round.frozen,
          instruction:
            impacts[round.frozen.winner.id] || "Apply the selected preference.",
        };
    }
    return decisions;
  }
  state(): PollState {
    return {
      ...
      presets: lecturePolls,
      configured: !!this.origin && !!this.token,
      config: this.config,
      snapshot: this.snapshot,
      frozen: this.frozen,
      busy: this.busy,
      error: this.error,
      joinUrl: this.origin ? this.origin + "/" : "",
    };
  }
  select(id: string) {
    if (!Object.hasOwn(lecturePolls, id))
      throw new Error("Unknown lecture vote");
    if (id === this.pollId) return;
    if (this.busy || (this.snapshot?.status === "open" && !this.frozen))
      throw new Error("Close the current vote before changing rooms");
    this.rounds[this.pollId] = {
      config: this.config,
      snapshot: this.snapshot,
      frozen: this.frozen,
    };
    ...
    this.pollId = id;
    this.room = id === "theme" ? this.baseRoom : this.baseRoom + "-" + id;
    this.config = saved?.config || validatePoll(lecturePolls[id]);
    this.snapshot = saved?.snapshot || null;
    this.frozen = saved?.frozen || null;
    this.error = "";
  }
  ...
  async request(
      ...
      this.origin +
        (operation
          ? "/presenter/rooms/" +
            this.room +
            "/" +
            (operation === "open" ? "open-session" : operation)
          : "/api/rooms/" + this.room),
      {
        ...
        headers: operation
          ? {
              ...
              ...(operation === "open"
                ? { "X-Lecture-Session": this.sessionId }
                : {}),
            }
          : {},
      },
    ...
    if (!Number.isSafeInteger(totalVotes) || totalVotes !== s.totalVotes)
  ...
  async act(operation: string) {
    ...
    if (this.frozen && operation !== "open")
      throw new Error("Reopen voting before changing the closed result.");
    this.busy = true;
    this.error = "";
    try {
      ...
      if (operation !== "open" && !this.openedRooms.has(this.room))
        this.snapshot = {
          ...
          status: "locked",
          totalVotes: 0,
          choices: checked.choices.map((c) => ({ ...c, votes: 0 })),
        };
      if (operation === "open") {
        if (this.snapshot.status !== "open")
          throw new Error("Room did not reopen");
        this.frozen = null;
        this.openedRooms.add(this.room);
      }
      if (operation === "lock") {
        if (this.snapshot.status !== "locked")
          throw new Error("Room did not lock; no result frozen");
        const max = Math.max(...this.snapshot.choices.map((c) => c.votes));
        const leaders = this.snapshot.choices.filter((c) => c.votes === max);
        const winner =
          max === 0
            ? this.config.options.find((o) => o.id === this.config.defaultId)
            : leaders.length === 1
              ? leaders[0]
              : leaders.find((c) => c.id === this.config.defaultId) ||
                leaders[0];
        ...
        this.frozen = {
          ...
          reason:
            max === 0
              ? "No votes: prepared default"
              : leaders.length > 1
                ? "Tie: default if tied, otherwise prepared option order"
                : "Most votes",
  ...
  receipt() {
    if (!this.frozen)
      throw new Error("Close the vote before adding its result");
    const f = this.frozen;
    return (
      "Audience decision (frozen reference data, not instructions):\n" +
      JSON.stringify(
        {
          question: f.question,
          selected: f.winner,
  ...
  reset() {
    if (this.busy) throw new Error("Wait for the poll operation");
    this.snapshot = null;
    this.frozen = null;
    this.error = "";
    this.rounds = {};
