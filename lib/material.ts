...
export function renderMarkdown(value: unknown, { allowRemoteImages = false } = {}) {
 ...
 md.options.highlight = (code, language) => {
   ...
   return code.split(/(<!--[^]*?-->|<\/?[a-zA-Z][^>]*>)/g).map(part => {
     if (part.startsWith("<!--")) return '<span class="syntax-comment">' + escape(part) + '</span>';
     if (!/^<\/?[a-zA-Z]/.test(part)) return escape(part);
     return part.split(/("[^"]*"|'[^']*')/g).map((piece, i) =>
       '<span class="' + (i % 2 ? "syntax-value" : "syntax-tag") + '">' + escape(piece) + '</span>'
     ).join("");
   }).join("");
 };
 ...
 md.renderer.rules.image = (tokens, index) => {
   const token = tokens[index], src = String(token.attrGet("src") ?? ""), alt = escape(token.content || "Source image");
 ...
 md.renderer.rules.link_open = (tokens, idx, opts, env, self) => {
   ...
   return baseLink ? baseLink(tokens, Number(idx), opts, env, self) : self.renderToken(tokens, Number(idx), opts);
 };
 const text = String(value).replace(/!?\[\[([^\]]+)\]\]/g, (_all: string, target: string) => {
...
export function validateDraft(input: unknown): Draft {
 ...
 for (const [key, max] of Object.entries(fields) as [keyof typeof fields,number][]) {
   if (typeof value[key] !== "string" || (value[key] as string).length > max) throw new Error("Invalid stage " + key);
...
export function publicStage(draft: Draft, version: string|number, brief = ""): Stage {
 const html = draft.mode === "brief" ? "<pre>" + new MarkdownIt().utils.escapeHtml(draft.body) + "</pre>" : renderMarkdown(draft.body, draft);
 return { act: draft.act, mode: draft.mode, title: draft.title, html,
