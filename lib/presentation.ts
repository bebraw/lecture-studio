import type {Theme,Note,PresentationDefinition,Step,FrozenPoll,BuildRun,BuildInput} from "../shared/models.ts";
...
export function parsePresentation(note:Note): PresentationDefinition{
 ...
 const value:PresentationDefinition=JSON.parse(raw?.match(/^\s*```json\s*\n([\s\S]*?)\n```\s*$/)?.[1]||"null");
 ...
 for(const s of value.steps){
   if(s.allowRemoteImages!==undefined&&typeof s.allowRemoteImages!=="boolean")throw new Error("Invalid remote image setting");
...
export class PresentationSession{
 definition:PresentationDefinition;path:string;current:string;history:string[];returns:{id:string;depth:number}[];decisions:Record<string,FrozenPoll>;runs:BuildRun[];defaults:Set<string>;loadedAt:string;

 constructor(definition:PresentationDefinition,path:string){this.definition=definition;this.path=path;this.current=definition.start;this.history=[];this.returns=[];this.decisions={};this.runs=[];this.defaults=new Set();this.loadedAt=new Date().toISOString();}
 step():Step{const step=this.definition.steps.find(s=>s.id===this.current);if(!step)throw new Error("Unknown current step");return step;}
 position(){
   const steps=this.definition.steps,path:string[]=[],seen=new Set<string>();let id:string|undefined=this.definition.start;
   while(id&&!seen.has(id)){seen.add(id);const s=steps.find(s=>s.id===id);if(!s||s.chapter==="References")break;path.push(id);id=s.next;}
   const currentIndex=path.indexOf(this.current);
   const anchor=currentIndex>=0?currentIndex:[...this.history].reverse().map(id=>path.indexOf(id)).find(i=>i>=0);
   return {number:steps.findIndex(s=>s.id===this.current)+1,total:steps.length,
     progress:currentIndex>=0?(currentIndex+1)/path.length:this.step().chapter==="References"?1:anchor===undefined?null:(anchor+1)/path.length};
 }
 ...
 resolve(){
   const s=this.step();const missing:string[]=[];const inputs:BuildInput[]=[];
   const additions=(s.uses||[]).map(dep=>{
     const p=this.definition.steps.find(x=>x.id===dep.poll);if(!p?.poll)throw new Error("Missing poll dependency");
...
state(){return {theme:this.definition.theme,preview:publicStage({...initialDraft(),mode:this.step().type==='build'?'brief':'material',title:this.step().title,body:this.step().type==='build'?this.resolve().prompt:(this.step().body||'')+(this.step().type==='poll'?'\n\n'+this.step().poll!.options.map(o=>o.label).join('\n\n'):''),source:this.step().source||'',allowRemoteImages:this.step().allowRemoteImages===true},this.loadedAt+'-'+this.current),outline:this.definition.steps.map(({id,title,type,chapter,next,related})=>({id,title,type,chapter,next,related})),title:this.definition.title,path:this.path,loadedAt:this.loadedAt,current:this.current,step:this.step(),related:(this.step().related||[]).map(id=>this.definition.steps.find(s=>s.id===id)!),canReturn:!!this.returns.length,canPrevious:!!this.history.length,resolved:this.resolve(),runs:this.runs};}
}
