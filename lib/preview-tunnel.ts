...
export class PreviewTunnel {
  ...
  constructor({
    binary = process.env.LECTURE_CLOUDFLARED_BIN || "cloudflared",
    spawnProcess = spawn,
    timeout = 30000,
  }: { binary?: string; spawnProcess?: SpawnTunnel; timeout?: number } = {}) {
    this.binary = binary;
  ...
  async open(value: string, studioOrigin: string) {
    ...
    if (this.target === target.origin && this.url)
      ...
      const ready = new Promise<string>((resolve, reject) => {
        ...
        const timer = setTimeout(
          () =>
            reject(
              new Error(
                "Preview sharing timed out; check cloudflared and retry",
              ),
            ),
          this.timeout,
        );
        ...
        const onData = (chunk: Buffer | string) => {
          ...
          publicOrigin =
            ...
            publicOrigin;
          if (publicOrigin && output.includes("Registered tunnel connection"))
            finish();
        };
        ...
        proc.once("error", () =>
          finish(
            new Error(
              "Cannot share preview: install cloudflared or set LECTURE_CLOUDFLARED_BIN",
            ),
          ),
        );
        proc.once("exit", () => {
          ...
          finish(new Error("Preview tunnel stopped"));
        });
      });
      this.pending = ready;
  ...
  publicUrl(value: string) {
    try {
      const target = new URL(value);
      return this.url && target.origin === this.target
