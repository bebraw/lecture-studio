...
export async function feedbackRequest(
  ...
  while (true) {
    ...
    if (size > 1000000) {
      await reader.cancel();
      throw new Error("Feedback response too large");
    }
    chunks.push(value);
  }
...
export function feedbackSlide(snapshot: FeedbackSnapshot, id?: string) {
  if (id) {
    ...
    return {
      ...
      title: item.text,
      html: "",
      source: "Audience question · selected by the lecturer",
    };
  }
  if (snapshot.config?.mode !== "words")
    throw new Error("Start a word collection first");
  ...
  return {
    ...
    html:
      '<div class="word-cloud" role="list" aria-label="Approved audience words">' +
      words
        .map(
          ([word, n]) =>
            '<span role="listitem" class="word-size-' +
            Math.min(4, Math.ceil((n / max) * 4)) +
            '">' +
            escape(word) +
            "<small> ×" +
            n +
        ...
        .join("") +
      "</div>",
    source: "Approved responses only · frozen when shown",
  };
}
