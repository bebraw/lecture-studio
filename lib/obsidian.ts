...
interface CodexConfig {mcp_servers?:Record<string,{url?:string;bearer_token_env_var?:string}>}
...
import { scopedPath, sections } from "./material.ts";

export async function readCodexConfig():Promise<CodexConfig> {
 try { return parse(await readFile(join(process.env.CODEX_HOME || join(homedir(), ".codex"), "config.toml"), "utf8")) as CodexConfig; }
 catch (caught) { const error = asError(caught); if (error.code === "ENOENT") return {}; throw new Error("Cannot read Codex configuration"); }
}
export function connectionConfig(config:CodexConfig, env = process.env) {
 ...
 return { url, headers: new Headers(token ? { Authorization: "Bearer " + token } : {}) };
}
...
export class ObsidianLibrary {
 client?:Client; connecting?:Promise<Client>; status = "Not connected";
 async connect() {
 ...
 async call(name:string, args:Record<string,unknown>) {
   ...
   try { return payload(await client.callTool({ name, arguments: args }, undefined, { timeout: 15000 }) as CallToolResult); }
 ...
 async list() {
   ...
   const files = Array.isArray(result) ? result : record(result).files;
 ...
 async read(path:string) {
   ...
   const content = typeof result === "string" ? result : record(result).content;
