...
export function connectionConfig(config: CodexConfig, env = process.env) {
  ...
  if (
    ...
    throw new Error("Obsidian must use a credential-free loopback MCP URL");
  const tokenName = entry.bearer_token_env_var || "OBSIDIAN_API_KEY";
  const token = env[tokenName];
  if (entry.bearer_token_env_var && !token)
    throw new Error(
      "Obsidian needs its configured token in the launching environment",
    );
  return {
    url,
...
export class ObsidianLibrary {
  ...
  async connect() {
    ...
    this.connecting = (async () => {
      ...
      try {
        ...
        throw new Error(
          "Obsidian is unavailable. Open the vault and check the MCP endpoint and token environment.",
        );
      }
    })();
    try {
  ...
  async call(name: string, args: Record<string, unknown>) {
    ...
    try {
      ...
      throw new Error(
        "Obsidian read failed. Check the connection; the projected stage has not changed.",
      );
    }
  }
  async list() {
    const result = await this.call("list_vault_files", {
      directory: scope,
      limit: 200,
    });
    ...
    return files
      ...
      .map((p) => ({
        path: scopedPath(p),
        label: p.slice(scope.length + 1).replace(/\.md$/, ""),
      }));
  }
  async read(path: string) {
    ...
    if (typeof content !== "string" || content.length > 150000)
