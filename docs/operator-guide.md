...
- **Ask a question** stays available throughout Live, including during word clouds, votes and demos. In **Responses**, select **Questions** to review the independent queue; the menu shows its pending count. Switching review modes does not change the audience activity. Live off closes submissions; Live on resumes questions. Pending questions remain private until **Discuss**. Use **Q&A desk** for pending, shortlisted, follow-up and completed groups. Arrow keys select; S shortlists, D discusses, A marks answered, F marks follow-up, R returns to the slide/demo, and Escape exits. Deploy the matching audience Worker before running the updated studio live.
...
- **Skip demo →** at an app checkpoint continues to the next slide while generation keeps running. Use the outline to revisit the checkpoint later; completion does not switch the projection. **Stop build** beside the navigation controls interrupts the active build without changing the slide or deleting its files. Wait for interruption to finish before starting another build; only one build runs at a time. An interrupted build can leave partial changes, so inspect the app before building on it.
...
Pending responses are private. Approved words may be projected and included as data in explicitly launched model builds. Queue expiry is 24 hours; saved projections, model conversations and generated artifacts have separate retention. Optional reply emails are visible only to the presenter and excluded from projection, public exports and AI context. Use **Reply later**, then select follow-ups and **Export selected follow-ups (.md)** before the displayed expiry, even after Live ends. Send replies separately from your email application.
