...
- **Ask a question** stays available throughout Live, including during word clouds, votes and demos. In **Responses**, select **Questions** to review the independent queue; the menu shows its pending count. Switching review modes does not change the audience activity. Live off closes submissions; Live on resumes questions. Pending questions remain private until **Discuss**. This requires the audience Worker with the `continuous-questions` capability; deploy it before running the updated studio live.
