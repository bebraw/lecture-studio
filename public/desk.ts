...
let state: DeskState,
  files: NoteFile[] = [],
  note: Note | undefined,
  selected: Note["sections"][number] | undefined,
  requestKey = "",
  modelKey = "",
  draftSequence = 0;
let previewKey = "";
let stageKey = "",
  stageReadPending = false;
...
function draft() {
  return {
    ...
    body: $("body").value,
    source: $("source").value,
    demoUrl: $("demo-url").value,
    allowRemoteImages: $("remote-images").checked,
  };
}
function fields(value: Draft) {
  for (const id of ["mode", "diagram", "title", "body", "source"] as const)
    $(id).value = value[id] || "";
  $("demo-url").value = value.demoUrl || "";
...
function preview(data: DeskState) {
  ...
  $("url-field").hidden = $("mode").value !== "demo";
}
async function saveDraft() {
  const sequence = ++draftSequence;
  const data = await call("draft", draft());
  if (sequence === draftSequence) preview(data);
  return data;
}
function beat() {
  const current =
...
function drawPlot() {
  ...
  if ($("live-next")) {
    $("live-next").textContent = beat().title;
    $("live-question").textContent = beat().question;
  }
  syncLectureSlide?.();
  $("acts").innerHTML = state.acts
    .map(
      (a, i) =>
        ...
        escape(a.id) +
        '" class="act ' +
        (a.id === state.activeAct ? "active" : "") +
        '"><span class="act-index">0' +
        (i + 1) +
        "</span><span><small>" +
        escape(a.era) +
        " · " +
      ...
      (button) =>
        (button.onclick = async () => {
          try {
            ...
            $("transition").textContent = beat().transition;
          } catch (caught) {
            const e = asError(caught);
            notice(e.message, true);
          }
  ...
  for (const file of files.filter((f) =>
    ...
    button.onclick = async () => {
      try {
        ...
        $("sections").innerHTML = note.sections
          .map(
            (section, i) =>
              '<option value="' +
              i +
          ...
          .join("");
        const preferred = note.sections.findIndex(
          (s) => s.heading.toLowerCase() === "stage block",
        );
        $("sections").value = String(Math.max(0, preferred));
        $("note-controls").hidden = false;
        selectSection();
      } catch (caught) {
        const e = asError(caught);
        notice(e.message, true);
...
function updateRuntime(data: DeskState) {
  ...
  if ($("rehearsal-status")) {
    ...
    $("rehearsal-status").textContent =
      job.status === "creating"
        ? "Preparing a fresh checkout and installing dependencies…"
        : job.status === "failed"
          ? (job.error ?? "")
          : job.status === "ready"
            ? "Fresh rehearsal ready."
            : "";
  ...
  state = {
    ...
    graphPoll: data.graphPoll,
    libraryStatus: data.libraryStatus,
    workspace: data.workspace,
    codex: data.codex,
    blank: data.blank,
    stage: data.stage,
    canReturnToMaterial: data.canReturnToMaterial,
    lastBrief: data.lastBrief,
  };
  ...
  if ($("live-progress"))
    $("live-progress").textContent = c.startedAt ? buildLabel(c) : "";
  $("codex-status").textContent = c.status;
  $("activity").textContent = buildLabel(c);
  $("workspace").textContent = data.workspace;
  $("send").disabled =
    c.status !== "ready" || data.rehearsalJob?.status === "creating";
  $("interrupt").disabled = !c.turnId;
  $("connect-codex").disabled = c.status !== "disconnected";
  $("disconnect-codex").disabled = c.status === "disconnected";
  ...
  $("notes-signal").title =
    ...
    (notesReady
      ? "ready"
      : /Unavailable|failed/i.test(data.libraryStatus)
        ? "unavailable"
        : "offline");
  ...
  $("load-library").textContent = notesReady
    ? "Refresh lecture notes"
    : "Connect Obsidian";
  $("stage-status").textContent = data.blank
    ? "Stage is blank."
    : "Published: " + data.stage.title;
...
function drawRequests(requests: ApprovalRequest[]) {
  ...
  for (const request of requests) {
    ...
    p.textContent = JSON.stringify(request.params, null, 2);
    panel.append(p);
    ...
    const choices: ["accept" | "answer" | "decline", string][] =
      request.method === "item/tool/requestUserInput"
        ...
        : request.method === "item/permissions/requestApproval"
          ? [["decline", "Decline broad permissions"]]
          : [
              ["accept", "Approve once"],
              ["decline", "Decline"],
            ];
    for (const [decision, label] of choices) {
      ...
      button.onclick = async () => {
        try {
          updateRuntime(
            await call("codex/answer", { id: request.id, decision, answers }),
          );
        } catch (caught) {
          const e = asError(caught);
          notice(e.message, true);
...
async function init() {
  ...
  $("open-stage").onclick = () =>
    window.open(stage.url, "_blank", "noopener,noreferrer");
  action("load-library", async () => {
    files = (await call("library")).files;
    listNotes();
    notice("Lecture notes are available. Nothing has been projected or sent.");
  });
  $("search").oninput = listNotes;
  $("sections").onchange = selectSection;
  action("use-section", async () => {
    if (!note || !selected) throw new Error("Select a note section");
    fields({
      ...draft(),
      mode: "material",
      title: note.title ?? "",
  ...
  action("publish", async () => {
    ...
    preview(data);
    updateRuntime(data);
    notice("Selected material is now on stage.");
  });
  action("blank", async () =>
    updateRuntime(await call("blank", { blank: !state.blank })),
  );
  action("save", async () => {
    await saveDraft();
    ...
    await call("save", {});
    notice(
      "Private draft saved locally. Agent output and credentials were not saved.",
    );
  });
  action("restore", async () => {
    state = await call("restore", {});
    fields(state.draft);
    $("brief").value = state.brief;
    drawPlot();
    preview(state);
    notice("Private draft restored. The projected stage has not changed.");
  });
  action("suggest-brief", async () => {
    $("brief").value = beat().brief;
    await call("brief", { brief: $("brief").value });
  });
  action("add-excerpt", async () => {
    if (!selected || !note)
      throw new Error("Select a section from a lecture note first.");
    $("brief").value +=
    ...
    await call("brief", { brief: $("brief").value });
    notice(
      "Only the selected section was added. Review the brief before sending.",
    );
  });
  action("show-brief", async () =>
    updateRuntime(await call("publish-brief", { brief: $("brief").value })),
  );
  action("show-sent-brief", async () =>
    updateRuntime(await call("publish-sent-brief", {})),
  );
  action("connect-codex", async () =>
    updateRuntime(await call("codex/connect", {})),
  );
  action("disconnect-codex", async () =>
    updateRuntime(await call("codex/disconnect", {})),
  );
  action("send", async () =>
    updateRuntime(
  ...
  action("interrupt", async () =>
    updateRuntime(await call("codex/interrupt", {})),
  );
  async function poll() {
    try {
      updateRuntime(await call("desk"));
    } catch (caught) {
      const e = asError(caught);
      notice(
        "Desk connection lost. Stage holds its last view. " + e.message,
        true,
      );
    } finally {
...
async function autoConnect() {
  const attempts: [string, () => Promise<void>][] = [
    ...
    [
      "Codex",
      async () => {
        if (state.codex.status === "disconnected")
          await call("codex/connect", {});
      },
    ],
  ];
  const results = await Promise.allSettled(
    attempts.map(([, connect]) => connect()),
  );
  try {
    updateRuntime(await call("desk"));
  } catch {
  ...
  if (failures.length)
    notice(
      "Automatic connection unavailable. Retry in Connections. " +
        failures.join(" · "),
      true,
    );
}
...
selectedTitle.id = "selected-note";
...
function setupConnections() {
  ...
  hint.textContent =
    "Connects automatically when this desk opens. Read-only lecture folder; coding starts only when you send a prompt.";
  notes.append(hint);
...
explorer = mountExplorer({
  host: query(".material-column", document),
  call,
  getScope: () => state.scope,
  onShow: async (material) => {
...
function setupModes() {
  ...
  modes.setAttribute("aria-label", "Desk mode");
  modes.innerHTML =
    '<button id="prepare-mode" aria-pressed="false">Prepare</button><button id="present-mode" aria-pressed="true">Present</button>';
  query(".top-actions", document).prepend(modes);
  const live = document.createElement("section");
  live.className = "live-overview";
  live.innerHTML =
  ...
  const audienceSlides: [string, string, string][] = [
    [
      ...
      "Understand how the web evolved—and explore where it might go by building an application together.",
    ],
    [
      "Opening · your experience",
      "When you use the web today, what feels unnecessarily difficult?",
      "Take at least two minutes to discuss with a neighbour. Choose one example to share.\n\nFinding information · Repeating information · Navigating interfaces · Knowing what to trust",
    ],
    [
      "Opening · shared experiment",
      "We will build one application—and change how we use it.",
      "You help choose the direction. An agent helps implement it. Together, we inspect what actually works.",
    ],
    [
      "Past · chapter",
      "Past — Documents become interactive",
      "How does a page communicate what we can do?",
    ],
    [
      "Past · links",
      "A link offers a next step.",
      "An address identifies a resource. A link lets us reach it.",
    ],
    [
      "Past · predict",
      "What will still work if we remove the styling and JavaScript?",
      "Make a prediction before we try it.",
    ],
    [
      "Past · forms",
      "Forms turn reading into action.",
      "A form communicates an action, its inputs, and where the request will go.",
    ],
    [
      "Past · enhancement",
      "Keep the capability. Add the convenience.",
      "Progressive enhancement starts with a working core, then adds presentation and richer interaction.",
    ],
    [
      "Past → present · improve",
      "The capability works. What would make the interaction better?",
      "Suggest one change—and explain who it would help.",
    ],
    [
      "Present · chapter",
      "Present — The browser becomes an application",
      "What do we gain when the interface responds without a new page?",
    ],
    [
      "Present · shared experiment",
      "One room. A changing result.",
      "Make a choice. Watch how the shared view responds.",
    ],
    [
      "Present · tradeoffs",
      "What does faster feedback add—and what does it hide?",
      "Compare the two versions. Name one benefit and one cost.",
    ],
    [
      "Present → future · another client",
      "A person can use this. What would another client need to understand it?",
      "Think about the available actions, required input, and resulting state.",
    ],
    [
      "Future · chapter",
      "Future — Who constructs the interface?",
      "Existing applications can become clearer to agents. Agents may also compose interfaces around services.\n\nTwo hypotheses that can coexist.",
    ],
    [
      "Future · stable or generated?",
      "Which applications need a stable interface, and which could use a generated one?",
      "Choose one example of each. What makes the difference?",
    ],
    [
      "Future · shared capability",
      "Can people and agents use the same underlying capability?",
      "Different interfaces need not mean independently maintained rules.",
    ],
    [
      "Future · context",
      "What context would improve this result—and what should remain private?",
      "Name one useful piece of context and one boundary you would set.",
    ],
    [
      "Closing · return",
      "If the interface changes, what should remain dependable?",
      "Return to your opening frustration. Does what we built help—or move the problem somewhere else?",
    ],
    [
      "Closing · next step",
      "What would you investigate or build next?",
      "One question. One counterexample. One practical experiment.",
    ],
  ];
  const slideActs = [
    "opening",
  ...
  const slideGuides = [
    "00–03 · Establish the two threads: web history and direction, demonstrated through agentic development.",
    "03–08 · Hear two examples. Remember them for the closing discussion. Use a show of hands if the room app is not ready.",
    "08–12 · Explain the prepared template and recorded context. Invite a visual-theme choice. Show the real prompt before sending it; build in the background, not automatically.",
    "12–15 · Start Document A when ready. Use the hypermedia definition and diagram while it runs. These are overlapping approaches, not replacement eras.",
    "15–20 · Show a real link, its URL and browser navigation. Connect distributed information to the audience’s experience, rather than listing dates.",
    "20–24 · Take predictions, then inspect the running result. If the build is unfinished, inspect the prepared baseline.",
    "24–29 · Start Document B using the prepared room. Explain native submission while it builds; inspect the actual request and response.",
    "29–34 · Use the enhancement definition and layers. Test without JavaScript. Discuss keyboard access; semantic HTML alone is not proof of accessibility.",
    "34–38 · Gather suggestions. Carry one into the Present prompt; do not promise every suggestion will be implemented.",
    "38–41 · Start the Present build. Show its prompt and explain what is preserved from the native form.",
    "41–49 · Invite predefined votes once deployed and reachable. Compare two browser views. If unavailable, demonstrate locally without pretending the audience is connected.",
    "49–54 · Give frameworks credit. Discuss state, latency, failure and discoverability through the actual app, not a technology list.",
    "54–58 · Inspect the form or shared contract. Distinguish explicit actions from behavior that must be inferred.",
    "58–62 · Start constrained Future composition when inputs are ready. Explain that the agent building this app and an agent using it are different roles.",
    "62–70 · Give pairs two minutes, then discuss examples while the build runs. Use the two-directions diagram. This is a position, not a proven forecast.",
    "70–76 · Inspect the generated result and shared constraints. Explain the drift risk of duplicate contracts, without claiming separate APIs are always wrong.",
    "76–83 · Show the actual context receipt. Distinguish aggregate audience choices from personal data; discuss profiling and incorrect assumptions.",
  ...
  for (const [after, act, title] of [
    ...
    [6, "forms", "Build · Add the native voting form"],
    [3, "document", "Build · Create the seminar document"],
  ] as const) {
  ...
  audienceSlides.splice(themeIndex, 0, [
    "Vote · theme",
    "Which visual theme should shape our app?",
    "Choose the visual direction we will use in the build.",
  ]);
  slideActs.splice(themeIndex, 0, "document");
  slideGuides.splice(
    themeIndex,
    0,
    "Open the prepared theme vote. Close it to freeze the choice before starting the build.",
  );
  const priorityIndex = audienceSlides.findIndex(
    (s) => s[1] === "Build · Compose a constrained interface",
  );
  audienceSlides.splice(priorityIndex, 0, [
    "Vote · priority",
    "What should the generated seminar view prioritize?",
    "Your choice sets the ordering and emphasis of the generated view.",
  ]);
  slideActs.splice(priorityIndex, 0, "agents");
  slideGuides.splice(
    priorityIndex,
    0,
  ...
  const openVote = document.createElement("button");
  openVote.id = "slide-vote-open";
  openVote.textContent = "Open voting";
  const closeVote = document.createElement("button");
  closeVote.id = "slide-vote-close";
  closeVote.textContent = "Close voting and continue";
  closeVote.className = "primary";
  $("next-beat").parentElement!.append(openVote, closeVote);
  const launch = document.createElement("button");
  launch.id = "start-slide-build";
  launch.className = "primary";
  launch.textContent = "Start this build";
  launch.hidden = true;
  $("next-beat").parentElement!.append(launch);
  const launchStatus = document.createElement("span");
  launchStatus.className = "small muted";
  launchStatus.id = "slide-build-status";
  launch.after(launchStatus);
  const guide = document.createElement("p");
  guide.className = "small muted";
  guide.id = "lecture-guide";
  live.lastElementChild!.append(guide);
  let slideIndex = 0,
    slideBusy = false,
    lectureReset: number | undefined;
  const startedSlides = new Set();
  const promptFor = (index: number) => {
    const base =
      state.acts.find((a) => a.id === buildSlides.get(index))?.brief || "";
    const relevant =
      buildSlides.get(index) === "agents"
        ? ["friction", "theme", "priority"]
        : ["friction", "theme"];
    return (
      base +
      ...
      relevant
        .map((id) => {
          ...
          return d
            ...
            : id +
                ": No frozen audience result yet. Keep the prepared project default; do not claim the audience selected it.";
        })
        .join("\n\n")
    );
  };
  updateBuildSlide = (c) => {
    ...
    const isVote = voteSlides.has(slideIndex),
      p = state.poll;
    openVote.hidden = closeVote.hidden = !isVote;
    openVote.textContent = p?.frozen ? "Reopen voting" : "Open voting";
    openVote.disabled =
      !p?.configured || p.busy || p.snapshot?.status === "open";
    closeVote.disabled = !p?.configured || p.busy;
    launch.hidden = !isBuild;
    launchStatus.hidden = !isBuild;
    launch.disabled = c.status !== "ready" || startedSlides.has(slideIndex);
    launchStatus.textContent = startedSlides.has(slideIndex)
      ? "Started · continue to the next discussion slide."
      : c.status === "ready"
        ? "Sends the prompt displayed on this slide."
        : "Connect Codex or finish the current turn before starting.";
  };
  ...
  action("start-slide-build", async () => {
    if (
      !buildSlides.has(slideIndex) ||
      startedSlides.has(slideIndex) ||
      state.codex.status !== "ready"
    )
      return;
    const index = slideIndex,
      brief = promptFor(index);
    ...
    startedSlides.add(index);
    $("brief").value = brief;
    updateRuntime(data);
    updateBuildSlide?.(data.codex);
  });
  syncLectureSlide = () => {
    if (lectureReset !== state.resetVersion) {
      slideIndex = 0;
      lectureReset = state.resetVersion;
      startedSlides.clear();
    }
    ...
    query(".section-label", live).textContent =
      ...
      " · " +
      label;
    $("live-next").textContent = title;
    $("live-question").textContent = buildSlides.has(slideIndex)
      ? promptFor(slideIndex)
      : body;
  ...
  query("p.small", live.lastElementChild!).textContent =
    "Previous and Next show the slide to the room. They never start a build.";
  const stagePanel = document.createElement("section");
  stagePanel.id = "current-stage-panel";
  stagePanel.innerHTML =
    '<div class="section-heading"><span class="section-label">ON STAGE</span><span id="live-progress" class="small muted"></span></div><div id="current-stage" class="preview stage-surface"></div>';
  ...
  cueControls.className = "cue-stage-controls";
  ...
  const placeStageControls = () => {
    ...
    (inPresent ? $("next-beat").parentElement : stageControls)!.append(
      $("blank"),
    );
  };
  const find = document.createElement("button");
  find.id = "find-material";
  find.textContent = "Find something…";
  find.className = "quiet";
  const done = document.createElement("button");
  done.id = "finish-finding";
  done.textContent = "Done · back to presenting";
  done.className = "primary";
  $("open-stage").before(find, done);
  const setMode = (mode: string) => {
    if (mode === "present") {
      document.querySelector<HTMLDetailsElement>(".connection")!.open = false;
      query("details.output", document).open = false;
    }
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && document.body.classList.contains("finding"))
      done.click();
  });
  for (const [id, step] of [
    ...
    action(id, async () => {
      await showLectureSlide(
        Math.min(audienceSlides.length - 1, Math.max(0, slideIndex + step)),
      );
    });
...
function setupPreviewShortcuts() {
  ...
  action("back-material", async () =>
    updateRuntime(await call("back-material", {})),
  );
}
function updatePreviewShortcuts(data: DeskState) {
  ...
  if (key !== previewKey) {
    ...
    $("preview-choice").innerHTML = urls
      .map(
        (url) =>
          '<option value="' + escape(url) + '">' + escape(url) + "</option>",
      )
      .join("");
  ...
  $("back-material").hidden =
    !data.canReturnToMaterial ||
    !["demo", "brief", "poll"].includes(data.stage.mode);
}
...
function setupPoll() {
  ...
  action("poll-add", async () => {
    ...
    await call("brief", { brief: $("brief").value });
    notice(
      "Frozen result added to the editable prompt. Nothing was sent or projected.",
    );
  });
}
function updatePoll(p: PollState) {
  ...
  if (configKey !== pollConfigKey) {
    pollConfigKey = configKey;
    $("poll-question").value = p.config.question;
    $("poll-options").value = p.config.options
      .map((o) => o.id + "|" + o.label)
      .join("\n");
    $("poll-default").value = p.config.defaultId;
  }
  $("poll-question-live").textContent = p.config.question;
  $("poll-join").hidden = !p.joinUrl;
  $("poll-join").href = p.joinUrl || "#";
  $("poll-status").textContent = !p.configured
    ? "Configure the public room connection in the server environment."
    : p.error ||
      (p.frozen
        ? "Closed · " + p.frozen.winner.label + " · " + p.frozen.reason
        : p.busy
          ? "Contacting audience room…"
          : p.snapshot?.status || "Ready to connect");
  $("poll-counts").textContent = (
    p.frozen?.choices ||
    p.snapshot?.choices ||
    []
  )
    .map((c) => c.label + ": " + c.votes)
    .join(" · ");
  ...
  $("poll-configure").disabled =
    p.busy || !!p.frozen || p.snapshot?.status === "open";
  if (
    p.snapshot?.status === "open" &&
    !p.frozen &&
    !p.busy &&
    !pollRefreshing &&
    Date.now() - lastPollRefresh > 3000
  ) {
    pollRefreshing = true;
    lastPollRefresh = Date.now();
    call("poll/refresh", {})
      .then(updateRuntime)
      .catch(() => {})
      .finally(() => (pollRefreshing = false));
  }
}
function setupRehearsals() {
  ...
  for (const [id, question] of [
    [
      "reset-lecture",
      "Return to the opening and discard the current unsaved lecture draft? This disconnects Codex and stops its owned process group. Project files and saved material stay intact.",
    ],
    [
      "new-rehearsal",
