...
export function applyTheme(element:HTMLElement,theme:Partial<Theme>={}){
 for(const [key,variable] of Object.entries({background:"paper",text:"ink",muted:"muted",accent:"accent",headingFont:"heading-font",bodyFont:"body-font",codeFont:"code-font"})){
   if(theme[key as keyof Theme])element.style.setProperty("--"+variable,theme[key as keyof Theme]!);else element.style.removeProperty("--"+variable);
 }
}
export function previewCandidates(messages:{text:string}[], ownOrigin:string) {
 const own = new URL(ownOrigin), found = new Set<string>();
 for (const message of messages || []) {
   for (const match of String(message.text || "").matchAll(/https?:\/\/[^\s<>"'`]+/g)) {
     try {
       const value = match[0].replace(/[),.;!]+$/, "");
       const url = new URL(value);
       if (!["localhost", "127.0.0.1", "[::1]"].includes(url.hostname)) continue;
       if (!url.port || url.port === own.port || url.username || url.password || url.search || url.hash) continue;
       if (/\/(?:api|json|devtools)(?:\/|$)/i.test(url.pathname)) continue;
       found.add(url.href);
     } catch { /* Incomplete streamed URLs are not actionable. */ }
   }
 }
 return [...found].slice(-8);
}
export function buildLabel(build:BuildState) {
 const labels:Record<string,string> = { working: build.activity || "Working", waiting: "Build paused", failed: "Build needs attention", ready: build.startedAt ? "Finished" : "Ready · nothing sent", disconnected: "Codex disconnected", connecting: "Connecting Codex" };
 const seconds = build.startedAt ? Math.max(0, Math.floor(((build.finishedAt || Date.now()) - build.startedAt) / 1000)) : 0;
 const timer = build.startedAt ? " · " + Math.floor(seconds / 60) + ":" + String(seconds % 60).padStart(2, "0") : "";
 return (build.status === "ready" && build.outcome === "interrupted" ? "Interrupted · review partial work" : labels[build.status] || "Idle") + timer;
}
export const escape = (text:unknown) => String(text ?? "").replace(/[&<>"']/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c as "&" | "<" | ">" | "\"" | "'"]));
export function auth(role:string) {
 const key = "lecture-studio-" + role;
 const token = document.querySelector<HTMLMetaElement>('meta[name="lecture-token"]')?.content || location.hash.slice(1) || sessionStorage.getItem(key) || "";
 if (location.hash) { sessionStorage.setItem(key, token); history.replaceState(null, "", location.pathname); }
 return token;
}
export async function api<P extends string>(token:string, path:P, value?:unknown):Promise<ApiResponse<P>> {
 const response = await fetch("/api/" + path, { signal: AbortSignal.timeout(65000), headers: { Authorization: "Bearer " + token, ...(value === undefined ? {} : { "content-type": "application/json" }) },
...
export async function renderDiagrams(container:HTMLElement) {
 ...
 try {
   ...
   const style=getComputedStyle(container),color=(name:string)=>style.getPropertyValue("--"+name).trim();
   ...
   for (const block of blocks) {
     ...
     const { svg } = await mermaid.render("diagram" + crypto.randomUUID().replaceAll("-", ""), text);
     if (parent?.isConnected) { const holder = document.createElement("div"); holder.className = "mermaid-graphic"; holder.innerHTML = svg; parent.replaceWith(holder); }
   }
 } catch { /* Keep the readable source when a diagram cannot render. */ }
}
