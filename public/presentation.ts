...
export function mountPresentations({call,update}:MountOptions){
 ...
 if(rehearsalControls)query(".session-actions",session).append(rehearsalControls);
 ...
 picker.append(name,setup);query(".brand",document).after(picker);setup.hidden=true;
 setup.append($("restart-presentation"));
 if(rehearsalControls)query(".connections-panel",document).append(rehearsalControls);
 session.remove();
 ...
 document.addEventListener("click",event=>{if(!picker.contains(event.target as Node))closePicker();});
 ...
 let data:DeskState, snapshot="", outlineKey="", previewKey="";
 ...
 setInterval(async()=>{
   ...
   refreshing=true;try{update(await call("presentation/poll-refresh",{}));}catch (caught) { const e = asError(caught);$("graph-status").textContent=e.message;}finally{refreshing=false;}
 },3000);
 async function run(op:string,body:unknown={}){
   try{const result=await call("presentation/"+op,body);update(result);return true;}
   catch (caught) { const e = asError(caught);$("graph-status").textContent=e.message;$("presentation-message").textContent=e.message;return false;}
 }
 ...
 document.addEventListener("keydown",event=>{
   if(!data?.presentation||event.defaultPrevented||event.altKey||event.ctrlKey||event.metaKey||event.shiftKey||(event.target as Element).closest("input,textarea,select,[contenteditable],dialog,[role=dialog],[role=slider],[role=tablist],[role=combobox],#connections"))return;
 ...
 $("presentations-list").onclick=async()=>{
   ...
   }catch (caught) { const e = asError(caught);$("presentation-message").textContent=e.message;}
 ...
 return (value:DeskState)=>{
   ...
   let chapter="";
   const addStep=(s:NonNullable<DeskState["presentation"]>["outline"][number],parent:string|null,target:HTMLElement,label=s.title)=>{
   ...
   const kind=shown?.blank?"Blank":({question:"Question",results:"Results",material:"Slide",brief:"Build prompt",demo:"App",diagram:"Diagram"}[shown?.projectionKind as "question"]||"Slide");
   projectionStatus.textContent=preparing()?"Private preview · audience waiting":"On stage: "+kind;
   projectionStatus.title=shown?.title||"";
   for(const id of ["question","results"]){($("graph-"+id) as HTMLButtonElement).disabled=!data.live;$("graph-"+id).setAttribute("aria-pressed",String(data.live&&shown?.projectionKind===id&&shown?.title===p.step.poll?.question));}
   applyTheme($("current-stage"),preparing()?p.theme:shown?.theme||p.theme);
   for(const button of all("[data-step-id]",outline))button.setAttribute("aria-current",button.dataset.stepId===p.current?"step":"false");
   ...
   for(const id of ["open","close"]){$("graph-"+id).hidden=p.step.type!=="poll";($("graph-"+id) as HTMLButtonElement).disabled=!data.graphPoll?.configured||!!data.graphPoll?.busy||(id==="open"?(!data.live||data.graphPoll?.snapshot?.status==="open"):(!!data.graphPoll?.frozen||data.graphPoll?.snapshot?.status!=="open"));}
 };
}
