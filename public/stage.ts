...
import { auth, api, surface, renderDiagrams, buildLabel, applyTheme } from "./shared.ts";
const token = auth("stage"); let version: string|number = -1;
...
async function poll() {
 try {
   ...
   if (state.version !== version) {
     ...
     query("#era",document).textContent = state.act.replaceAll("-", " ").toUpperCase();
     query("#source-credit",document).textContent = state.source;
     void renderDiagrams(query("#stage-content",document));
   }
   query("#build-signal",document).textContent = buildLabel(state.build ?? {status:"ready"});
   query("#stage-connection",document).textContent = "";
 } catch {
   query("#stage-connection",document).textContent = "Stage connection lost · holding the last view";
