...
const pair=(name,definition,diagram)=>[[definition,"Projection/"+name+".md","Slide: Definition"],[diagram,"Projection/"+name+".md","Slide: Image"]];
...
const projectable=s=>/^Slide: (Definition|Image|Quote)$/.test(s.heading);
export function mountExplorer({host,call,onShow,getScope}) {
 ...
 panel.querySelector(".section-heading").append($("explore-show"));
 ...
 $("explore-save").onclick=()=>{
   try{
     ...
     custom[cue.id]=rows;localStorage.setItem("lecture-projection-shortcuts",JSON.stringify(custom));draw();status("Shortcuts saved for this cue.");
   }catch(e){status(e.message);}
 };
 ...
 return {setCue(next){if(cue?.id===next.id)return;cue=next;request++;searchRequest++;note=undefined;section=undefined;$("explore-selection").hidden=true;$("explore-show").disabled=true;$("explore-results").replaceChildren();status("Choose a definition or visual. Nothing is projected until you show it.");draw();}};
}
