...
export function mountFeedback({call,update}:MountOptions){
 ...
 const $=byId;let snapshot:FeedbackSnapshot|null=null,busy=false,listKey="",revision=0;
 ...
 const render=(value:FeedbackSnapshot)=>{
   ...
   query("summary",menu).textContent="Responses · "+items.filter(x=>x.status==="pending").length;
 ...
 const act=async(action:string,id?:string)=>{
   ...
   try{
     ...
     if("projection" in result)update(result);else render(result);
     $("feedback-error").textContent="";
   }catch (caught) { const e = asError(caught);$("feedback-error").textContent=e.message;}finally{busy=false;}
 };
 for(const [id,action] of [["start","start"],["close","close"],["show","show-cloud"],["return","return"]])$("feedback-"+id).onclick=()=>void act(action);
 $("feedback-items").onclick=e=>{const button=(e.target as Element).closest<HTMLButtonElement>("button[data-action]");if(button)void act(button.dataset.action ?? "",button.dataset.id);};
 document.addEventListener("keydown",e=>{if(e.key==="Escape"&&menu.open){menu.open=false;query("summary",menu).focus();}});
 document.addEventListener("click",e=>{if(!menu.contains(e.target as Node))menu.open=false;});
 async function refresh(){
   try{if(!busy){const started=revision;const value=await call("feedback");if(!busy&&revision===started){if("config" in value)render(value);}}}
   catch (caught) { const e = asError(caught);if(menu.open)$("feedback-error").textContent=e.message;}
