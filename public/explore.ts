...
const pair=(name:string,definition:string,diagram:string):Shortcut[]=>[[definition,"Projection/"+name+".md","Slide: Definition"],[diagram,"Projection/"+name+".md","Slide: Image"]];
const web=()=>pair("Web foundations","What is hypermedia?","How links connect resources");
const defaults:Record<string,Shortcut[]>={
 opening:web(),
 document:web(),
 forms:pair("Progressive enhancement","What is progressive enhancement?","HTML → CSS → JavaScript layers"),
 application:pair("Browser applications","Updating a page without reloading","Browser ↔ server: partial updates"),
 agents:pair("Agentic directions","Two possible agentic directions","Provider-designed vs agent-composed UI"),
 context:pair("Context boundaries","What is a context receipt?","Which context reaches the agent?"),
 synthesis:pair("Shared capability","One capability, multiple clients","People, assistive tools, and agents")
};
const projectable=(s:{heading:string})=>/^Slide: (Definition|Image|Quote)$/.test(s.heading);
export function mountExplorer({host,call,onShow,getScope}:{host:HTMLElement;call:ApiClient;onShow:(value:Partial<Draft>)=>unknown;getScope:()=>string}) {
 ...
 const presenter=document.createElement("p");presenter.id="explore-presenter";presenter.className="small muted";
 $("explore-selection").prepend(presenter);
 let cue:{id:string;title:string}|undefined,notes:NoteFile[]=[],note:(Note & {credit?:string})|undefined,section:Note["sections"][number]|undefined,request=0,searchRequest=0,custom:Record<string,Shortcut[]>={};
 try{custom=JSON.parse(localStorage.getItem("lecture-projection-shortcuts")||"{}");}catch{}
 ...
 async function read(path:string,heading?:string){
   const id=++request;note=undefined;section=undefined;$("explore-selection").hidden=true;status("Reading lecture note…");$("explore-show").disabled=true;
   try{
     ...
     if(!eligible.length)throw new Error("No projection-ready material in this note. Add a Slide: Definition, Slide: Image, or Slide: Quote section in Prepare.");
     note={...result,sections:eligible};$("explore-title").textContent=note.title ?? "";
     presenter.textContent="Presenter cue · "+(result.sections.find(s=>s.heading==="Presenter cue")?.body||"");
     note.credit=result.sections.find(s=>s.heading==="Source")?.body||note.path;
   ...
   }catch (caught) { const e = asError(caught);if(id===request)status(e.message);}
   finally{if(id===request)$("explore-show").disabled=!section;}
 }
 function select(){if(!note)return;
 ...
 async function search(query:string){
   ...
   try{
     ...
     const matches=data.matches.filter(match=>projectable({heading:match.section}));
     ...
     status(matches.length+" projection-ready sections"+(data.unavailable?" · "+data.unavailable+" notes unavailable":"")+". Select one to read privately.");
   }catch (caught) { const e = asError(caught);if(id===searchRequest)status(e.message);}
 }
 function draw(){
   $("explore-cue").textContent=cue?.title ?? "";
   $("explore-shortcuts").replaceChildren();
   for(const item of mappings().filter(item=>Array.isArray(item)&&typeof item[1]==="string"&&!item[1].startsWith("?")).slice(0,3)){
 ...
 $("explore-show").onclick=async()=>{
   ...
   $("explore-show").disabled=true;
   try{await onShow({title:note.title,body:section.body,source:(note.credit ?? "").slice(0,500)});status("Excerpt shown to the room. No coding prompt sent.");}
   catch (caught) { const e = asError(caught);status(e.message);}
 ...
 $("explore-save").onclick=()=>{
   try{
     ...
     if(rows.length>3 || rows.some(m=>m.length<2||m.length>3||!m[0]||!m[1]||m[0].length>60||m[1].length>200||m[1].startsWith("/")||m[1].includes("..")||m[1].includes("\\")))throw new Error("Use up to three labeled shortcuts inside the lecture folder.");
     custom[cue!.id]=rows as Shortcut[];localStorage.setItem("lecture-projection-shortcuts",JSON.stringify(custom));draw();status("Shortcuts saved for this cue.");
   }catch (caught) { const e = asError(caught);status(e.message);}
 };
 $("explore-defaults").onclick=()=>{delete custom[cue!.id];localStorage.setItem("lecture-projection-shortcuts",JSON.stringify(custom));draw();};
 return {setCue(next:{id:string;title:string}){if(cue?.id===next.id)return;cue=next;request++;searchRequest++;note=undefined;section=undefined;$("explore-selection").hidden=true;$("explore-show").disabled=true;$("explore-results").replaceChildren();status("Choose a definition or visual. Nothing is projected until you show it.");draw();}};
}
