...
The public app must expose these rooms and matching option labels before rehearsal. The studio checks them before opening or locking; it never seeds, clears or resets public votes. No public rooms were changed as part of this implementation.
...
Without a frozen result, the prompt explicitly retains the prepared project default and does not claim audience endorsement. Builds still require Start this build. Frozen rounds survive navigation in server memory, but not a server restart or lecture reset. Resetting the lecture does not clear public room counts.
