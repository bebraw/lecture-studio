...
export function createStudio({ library = new ObsidianLibrary(), bridge = new CodexBridge(), poll = new AudiencePoll(), workspace = resolve(root, "../webdev-rehearsal-studio"), rehearsals = new Rehearsals(resolve(root, ".local/rehearsals")), port = 4317, host = "127.0.0.1", persist = true } = {}) {
 ...
 const publicState = () => {
   ...
   const projected = pollOnStage ? publicStage({ ...initialDraft(), mode: "material", title: p.config.question, body: p.config.options.map(o => o.label + (pollResults ? ": " + (counts?.choices.find(c=>c.id===o.id)?.votes || 0) : "")).join("\n\n") + "\n\n" + (pollResults && p.frozen ? "Selected: " + p.frozen.winner.label + " — " + p.frozen.reason : (p.configured ? "" : "Audience room is not configured. Discuss the choices together.")), source: p.frozen ? "Voting closed" : "Audience vote" }, String(version) + "-poll-" + (counts?.revision || 0) + "-" + !!p.frozen + "-" + pollResults) : stage;
   ...
   return { ...projected,slidePosition:presentation?.position()||null,live:true, projectionKind:pollOnStage?(pollResults?"results":"question"):stage.mode, theme:stage.theme, blank, build: { activity, status: c.status, outcome: ["completed", "failed", "interrupted"].includes(c.outcome) ? c.outcome : null, startedAt: c.startedAt || null, finishedAt: c.finishedAt || null } };
 };
