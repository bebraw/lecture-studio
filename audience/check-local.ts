...
const admin = async (room: string, action: string) => {
  const r = await fetch(origin + "/presenter/rooms/" + room + "/" + action, {
    method: "POST",
...
for (const room of [
  ...
  const unauthorized = await fetch(
    origin + "/presenter/rooms/" + room + "/open",
    { method: "POST" },
  );
  assert.equal(unauthorized.status, 401);
  const seeded = await admin(room, "seed");
  await admin(room, "lock");
  const page = await fetch(origin + "/rooms/" + room);
  assert.equal(page.status, 200);
  assert.match(await page.text(), /<form/);
  const send = (choice: string, from = origin, cookie = "") =>
    fetch(origin + "/rooms/" + room, {
      method: "POST",
      headers: {
        origin: from,
        cookie,
  ...
  assert.equal(
    (await send(seeded.choices[0].id, "https://unrelated.invalid")).status,
    403,
  );
  assert.equal((await send("invalid-option")).status, 400);
  const vote = await send(seeded.choices[0].id);
  ...
  assert.equal(repeated.totalVotes, result.totalVotes);
  assert.equal(repeated.status, "locked");
  console.log(
    room +
      ": form, authorization, origin checks, replacement and locking passed",
  );
}
