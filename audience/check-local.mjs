...
const admin=async(room,action)=>{const r=await fetch(origin+"/presenter/rooms/"+room+"/"+action,{method:"POST",headers:{authorization:"Bearer "+token}});assert.equal(r.status,200);return r.json();};
for(const room of ["webdev-2026-friction","webdev-2026","webdev-2026-priority"]){
 ...
 const send=(choice,from=origin,cookie="")=>fetch(origin+"/rooms/"+room,{method:"POST",headers:{origin:from,cookie,"content-type":"application/x-www-form-urlencoded"},body:"choice="+choice,redirect:"manual"});
