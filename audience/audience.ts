...
import {surface,applyTheme,renderDiagrams,buildLabel,escape} from "../public/shared.ts";
const main=query("#stage-content",document),notice=query("#stage-connection",document);
...
let feedbackConfig:FeedbackConfig|null=null,feedbackBusy=false;
async function refreshFeedback(){
 try{
   const response=await fetch("/api/feedback",{cache:"no-store",signal:AbortSignal.timeout(8000)});
   if(!response.ok)throw new Error();
   const config=await response.json();
   if(config?.round!==feedbackConfig?.round){query("form",feedback).reset();query("#feedback-notice",feedback).textContent="";}
   feedbackConfig=config;feedback.hidden=!config?.open;
   if(config?.open){
     query("summary",feedback).textContent=config.mode==="words"?"Add words":"Ask a question";
     query("#feedback-label",feedback).textContent=config.prompt;
     query("textarea",feedback).maxLength=config.mode==="words"?32:400;
   }
...
query("form",feedback).onsubmit=async event=>{
 ...
 feedbackBusy=true;const button=query("button",feedback);button.disabled=true;
 try{
   const response=await fetch("/api/feedback",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({round:feedbackConfig.round,text:query("textarea",feedback).value}),signal:AbortSignal.timeout(8000)});
   const result=await response.json();if(!response.ok)throw new Error(result.error||"Not confirmed");
   query("textarea",feedback).value="";
   query("#feedback-notice",feedback).textContent="Sent privately. The lecturer chooses what to show.";
 }catch (caught) { const e = asError(caught);query("#feedback-notice",feedback).textContent=e.message;}
...
async function refresh(){
 try{
   ...
   if(!submitting&&next!==key){
     ...
     query("#era",document).textContent=poll?"VOTE":stage?.act?.replaceAll("-"," ").toUpperCase()||"";
     query("#source-credit",document).textContent=poll?"Voting stays open until the lecturer closes it.":stage?.source||"";
     void renderDiagrams(main);
   }
   query("#build-signal",document).textContent=stage?.build?.startedAt?buildLabel(stage.build):"";
   notice.textContent=voteError;
 }catch{notice.textContent="Connection lost · holding the last view";}
}
main.addEventListener("submit",async event=>{
 event.preventDefault();if(submitting)return;
 const form=event.target as HTMLFormElement,button=query("button",form);submitting=true;button.disabled=true;
 try{
   const response=await fetch(form.action,{method:"POST",body:new URLSearchParams([...new FormData(form)].map(([key,value])=>[key,String(value)])),signal:AbortSignal.timeout(8000)});
   if(!response.ok)throw new Error();
