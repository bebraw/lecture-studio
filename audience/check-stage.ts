...
const admin = async (id: string, op: string) => {
  const response = await fetch(origin + "/presenter/rooms/" + id + "/" + op, {
    method: "POST",
    headers,
  });
...
const publish = async (title: string) => {
  const response = await fetch(origin + "/presenter/stage", {
    method: "POST",
    headers,
    body: JSON.stringify({
      title,
      ...
      version: title,
      theme: { background: "#ffffff", headingFont: "Verdana, sans-serif" },
      notes: "PRIVATE",
    }),
  });
  assert.equal(response.status, 200);
...
assert.equal(
  (await fetch(origin + "/presenter/stage", { method: "POST", body: "{}" }))
    .status,
  401,
);
...
try {
  ...
  await page.getByRole("heading", { name: "A shared stage" }).waitFor();
  await admin("webdev-2026", "open");
  assert.equal((await stop()).status, 409);
  await page
    .getByRole("heading", { name: "Which visual theme should shape our app?" })
    .waitFor();
  await page.getByLabel("Editorial", { exact: true }).check();
  await publish("The projector moved on");
  await page.waitForTimeout(1800);
  assert.equal(
    await page.getByLabel("Editorial", { exact: true }).isChecked(),
    true,
  );
  await page.getByRole("button", { name: "Vote", exact: true }).click();
  await page
    .getByRole("button", { name: "Vote saved · change vote" })
    .waitFor();
  assert.equal(page.url(), origin + "/");
  await page.screenshot({
    path: new URL("../test-results/audience-vote.png", import.meta.url)
      .pathname,
  });
  ...
  await page.screenshot({
    path: new URL("../test-results/audience-stage.png", import.meta.url)
      .pathname,
  });
  assert.equal((await stop()).status, 200);
  assert.deepEqual(await (await fetch(origin + "/api/audience")).json(), {
    stage: null,
    poll: null,
  });
  await page
  ...
  await page.getByRole("heading", { name: "Broadcast resumed" }).waitFor();
  console.log(
    "Shared stage, private-field exclusion, independent voting, focus preservation, vote replacement form, return to stage and mobile theme passed.",
  );
} finally {
  await browser.close();
  await admin("webdev-2026", "lock");
