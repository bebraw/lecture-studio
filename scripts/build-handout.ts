...
const slides = deck.steps
  .map((step, index) => {
    let body = renderMarkdown(step.body || "", {
    ...
    const activity =
      step.poll || step.wordCloud
        ...
        : "";
    return `<section class="slide" id="${escape(step.id)}"><p class="eyebrow">${escape(step.chapter || "Opening")} · ${index + 1}/${deck.steps.length}</p><h2>${escape(step.title)}</h2>${activity}${body}${options}${step.source ? `<footer>${citations(step.source)}</footer>` : ""}</section>`;
  })
  .join("\n");
const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(deck.title)} · Slides</title><link rel="stylesheet" href="/slides.css"></head><body>
<header class="intro"><p class="eyebrow">Lecture reading copy · 16 September 2026</p><h1>${escape(deck.title)}</h1><p>Juho Vepsäläinen</p><p>Revisit the slides, follow the sources, and keep a copy.</p><nav aria-label="Lecture sections">${[...chapters].map(([chapter, id]) => `<a href="#${escape(id)}">${escape(chapter)}</a>`).join(" ")}</nav><button id="print-slides" type="button" hidden>Print / save as PDF</button><noscript><p>Use your browser’s Print command to save a PDF. Diagrams are shown as source text without JavaScript.</p></noscript></header>
...
await writeFile(new URL(".local/browser/public/slides.html", root), html);
