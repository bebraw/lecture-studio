...
export function pollFixture(){
 ...
 return {poll,calls,votes:(counts:number[])=>{snapshot={...snapshot,revision:snapshot.revision+1,choices:snapshot.choices.map((c,i)=>({...c,votes:counts[i]})),totalVotes:counts.reduce((a,b)=>a+b,0)};}};
}
test("open, aggregate, lock, freeze, receipt; no votes or resets sent",async()=>{
 ...
 assert.ok(calls.filter(c=>c.init.method==="GET").every(c=>!new Headers(c.init.headers).get("Authorization")));
});
...
test("lecture reset rotates the session; reopen retains it and refresh hides old counts",async()=>{
 ...
 await poll.act("open");const first=new Headers(calls.at(-1)!.init.headers).get("X-Lecture-Session");
 await poll.act("lock");await poll.act("open");
 assert.equal(new Headers(calls.at(-1)!.init.headers).get("X-Lecture-Session"),first);
 await poll.act("lock");poll.reset();await poll.act("open");
 assert.notEqual(new Headers(calls.at(-1)!.init.headers).get("X-Lecture-Session"),first);
});
