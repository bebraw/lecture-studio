...
test("new lecture clears votes atomically; reopen and retry preserve them",async()=>{
 ...
 const ctx={blockConcurrencyWhile(fn:()=>unknown){fn();},storage:{
  sql:{exec(query:string,...args:import("node:sqlite").SQLInputValue[]){
   if(query.includes("CREATE TABLE")){db.exec(query);return;}
   const rows=db.prepare(query).all(...args);return {toArray:()=>rows,one:()=>rows[0]};
  }},
  transactionSync(fn:()=>unknown){db.exec("BEGIN");try{fn();db.exec("COMMIT");}catch (caught) { const e = asError(caught);db.exec("ROLLBACK");throw e;}}
 }};
 try{
