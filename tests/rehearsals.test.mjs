...
import { Rehearsals, START_COMMIT } from "../lib/rehearsals.mjs";
...
test("numbered checkouts retain earlier attempts and verify the pinned starter", async () => {
 ...
 const manager = new Rehearsals(root, async (bin, args) => {
   calls.push([bin, ...args]); return {stdout: args[0] === "rev-parse" ? START_COMMIT : ""};
 });
 const one = await manager.create(), two = await manager.create();
 assert.ok(one.endsWith("rehearsal-001")); assert.ok(two.endsWith("rehearsal-002"));
 await access(one); assert.equal(await manager.current("fallback"), two);
...
test("reset and fresh rehearsal are confirmed, private, and preserve the workspace on setup failure", async t => {
 let resolveSetup;
 const rehearsals = { create: () => new Promise(resolve=>resolveSetup=resolve) };
 const {studio,address,bridge} = await fixture({rehearsals});
 t.after(()=>studio.server.close());
 const post=async(path,body,token=address.deskToken)=>{
 ...
 const desk=async()=> (await fetch(address.origin+"/api/desk",{headers:{Authorization:"Bearer "+address.deskToken}})).json();
 assert.equal((await post("reset-lecture",{})).status,400);
 assert.equal((await post("new-rehearsal",{confirm:true},address.stageToken)).status,401);
 ...
 assert.equal(reset.workspace,old.workspace); assert.equal(reset.activeAct,"opening");
 assert.equal(reset.codex.messages.length,0); assert.equal(reset.codex.status,"disconnected");
 ...
 rehearsals.create=async()=>{throw new Error("offline")};
 await post("new-rehearsal",{confirm:true});
 await new Promise(resolve=>setTimeout(resolve,10));
 const failed=await desk();
 assert.equal(failed.rehearsalJob.status,"failed"); assert.equal(failed.workspace,"/fresh/rehearsal-001");
});
