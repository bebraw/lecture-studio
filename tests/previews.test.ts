...
test("preview handoff preserves published material and an unrelated private draft", async t => {
 ...
 const call=async(path:string,body:unknown,token=address.deskToken)=>{
...
test("live preview shares HTTPS with audience while keeping local stage URL",async t=>{
 ...
 const sent:Partial<Stage>[]=[];let shared="",closed=false;
 const tunnel={pending:null,async open(url:string){shared=url;return url;},publicUrl(url:string){return shared===url?"https://demo.trycloudflare.com/":url;},close(){closed=true;shared="";}};
 const poll=new AudiencePoll({origin:"https://audience.invalid",token:"test",fetcher:async(_url,init)=>{sent.push(JSON.parse(String(init.body)));return new Response(null,{status:204});}});
 const {studio,address}=await fixture({poll,previewTunnel:tunnel});t.after(()=>studio.server.close());
 const call=async(path:string,body:unknown)=>{const r=await fetch(address.origin+"/api/"+path,{method:"POST",headers:{Origin:address.origin,Authorization:"Bearer "+address.deskToken,"content-type":"application/json"},body:JSON.stringify(body)});assert.equal(r.status,200);return r.json();};
