...
test("audience sync serializes writes and coalesces intermediate slides",async()=>{
 const sent:Partial<Stage>[]=[],releases:(()=>void)[]=[];
 const sync=new AudienceStageSync({origin:"https://audience.invalid",token:"test",fetcher:async(_url,options)=>{sent.push(JSON.parse(String(options.body)));await new Promise<void>(resolve=>releases.push(resolve));return new Response(null,{status:204});}});
