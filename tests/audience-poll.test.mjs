...
export function pollFixture(){
 ...
 const poll=new AudiencePoll({origin:"https://audience.example",token:"private-presenter-secret",fetcher:async(url,init)=>{
   calls.push({url,init});
   if(url.endsWith("/open"))snapshot={...snapshot,status:"open",revision:snapshot.revision+1};
...
test("zero votes and ties are deterministic and labeled",async()=>{
 ...
 const b=pollFixture();b.votes([0,2,2]);await b.poll.act("lock");assert.equal(b.poll.frozen.winner.id,"retro-web");assert.match(b.poll.frozen.reason,/Tie/);
});
