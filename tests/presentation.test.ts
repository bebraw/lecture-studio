...
import {parsePresentation,PresentationSession,parseTheme} from "../lib/presentation.ts";
...
const note=(value:unknown)=>({sections:[{heading:"Presentation",body:"```json\n"+JSON.stringify(value)+"\n```"}]});
...
test("remote slide images require an explicit boolean opt-in",()=>{
 const d={version:1,title:"Images",start:"image",steps:[{id:"image",type:"material",title:"Browser",body:"![Browser](https://www.w3.org/browser.gif)",allowRemoteImages:false as unknown}]};
...
test("presentation snapshot, detour return and explicit defaults",()=>{
 ...
 p.decisions.poll={winner:{id:"two",label:"Two"},revision:7,question:"Theme?",reason:"votes",capturedAt:new Date().toISOString(),status:"locked",totalVotes:1,choices:[{id:"two",label:"Two",votes:1}]};
