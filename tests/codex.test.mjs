...
function backend() {
 const proc = new EventEmitter(), sent = [];
 ...
 proc.stdin=new Writable({write(chunk,_encoding,done){
   ...
   if(message.id!==undefined && message.method && result!==null) queueMicrotask(()=>proc.stdout.write(JSON.stringify({id:message.id,result})+"\n"));
   done();
 }});
 return {proc,sent};
}
test("Codex uses reviewed text, workspace sandbox, automatic approval review and one active turn",async()=>{
 ...
 try{
   ...
   bridge.receive({id:"input",method:"item/tool/requestUserInput",params:{questions:[{id:"q"}]}});
